...
package Gtk.Clipboard is
   ...
   --  if retrieving the given target failed. If non-null, this value must be
   --  freed with Gtk.Selection_Data.Free when you are finished with it.
   --  "target": an atom representing the form into which the clipboard owner
   --  should convert the selection.
   ...
   --  object which must be disposed with g_object_unref, or null if
   ...
   --  the wait.
   --  be freed with g_free, or null if retrieving the selection data failed.
   --  (This could happen for various reasons, in particular if the clipboard
   --  was empty or if the contents of the clipboard could not be converted
   --  into text form.)
   ...
   --  null-terminated array of strings which must be freed with g_strfreev,
   ...
   --  already exists, a new one will be created. Once a clipboard object has
   --  been created, it is persistent and, since it is owned by GTK+, must not
   ...
   --  http://www.freedesktop.org/Standards/clipboards-spec</ulink> for a
   ...
   --  Since: gtk+ 2.2
   --  "display": the display for which the clipboard is to be retrieved or
   --  created
