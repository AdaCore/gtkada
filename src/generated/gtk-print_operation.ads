...
--  to implement a good print dialog. On such platforms, `GtkPrintOperation`
--  uses the native print dialog. On platforms which do not provide a native
--  print dialog, GTK uses its own, see [classGtk.PrintUnixDialog].
--
--  The typical way to use the high-level printing API is to create a
--  `GtkPrintOperation` object with [ctorGtk.PrintOperation.new] when the user
--  selects to print. Then you set some properties on it, e.g. the page size,
--  any [classGtk.PrintSettings] from previous print operations, the number of
--  pages, the current page, etc.
--
--  Then you start the print operation by calling
--  [methodGtk.PrintOperation.run]. It will then show a dialog, let the user
...
--  [signalGtk.PrintOperation::draw-page], which you are supposed to handle and
--  render the page on the provided [classGtk.PrintContext] using Cairo.
--
...
--  [methodGtk.PrintOperationPreview.is_selected] are useful when implementing
--  a print preview.
...
package Gtk.Print_Operation is
   ...
   --
   --  A value of this type is returned by [methodGtk.PrintOperation.run].
   ...
   --  [signalGtk.PrintOperation::paginate] or
   --  [signalGtk.PrintOperation::draw-page] signal handler to stop the
   ...
   --  thread). If [methodGtk.PrintOperation.set_defer_drawing] was called
   ...
   --  This page setup will be used by [methodGtk.PrintOperation.run], but it
   --  can be overridden on a per-page basis by connecting to the
   --  [signalGtk.PrintOperation::request-page-setup] signal.
   ...
   --  Gtk.Print_Operation.Print_Operation_Result_Error.
   --  It can be called either after [methodGtk.PrintOperation.run] returns,
   --  or in the [signalGtk.PrintOperation::done] signal handler.
   ...
   --  by [methodGtk.PrintOperation.set_n_pages] in a handler for the
   --  [signalGtk.PrintOperation::begin-print] signal.
   ...
   --  (Gtk.Enums.Print_Status_Preparing), so this function should never be
   --  called before the data generation phase
   --  (Gtk.Enums.Print_Status_Generating_Data). You can connect to the
   --  [signalGtk.PrintOperation::status-changed] signal and call
   --  Gtk.Print_Operation.Get_N_Pages_To_Print when print status is
   --  Gtk.Enums.Print_Status_Generating_Data.
   ...
   --  print operation, see [methodGtk.PrintOperation.run].
   --  @param Print_Settings `GtkPrintSettings`

   function Get_Status
      (Self : not null access Gtk_Print_Operation_Record)
       return Gtk.Enums.Gtk_Print_Status;
   ...
   --  Use [methodGtk.PrintOperation.get_status] to obtain a status value that
   ...
   --  Gtk.Enums.Print_Status_Finished or
   --  Gtk.Enums.Print_Status_Finished_Aborted.
   ...
   function Run
      (Self   : not null access Gtk_Print_Operation_Record;
       ...
       Error  : out Glib.Error.GError) return Gtk_Print_Operation_Result;
   --  Runs the print operation.
   --  Normally that this function does not return until the rendering of all
   --  pages is complete. You can connect to the
   --  [signalGtk.PrintOperation::status-changed] signal on Op to obtain some
   --  information about the progress of the print operation.
   --  Furthermore, it may use a recursive mainloop to show the print dialog.
   --  If you set the [Gtk.PrintOperation:allow-async] property, the operation
   --  will run asynchronously if this is supported on the platform. The
   --  [signalGtk.PrintOperation::done] signal will be emitted with the result
   --  of the operation when the it is done (i.e. when the dialog is canceled,
   --  or when the print succeeds or fails).
   --  ```c if (settings != NULL) gtk_print_operation_set_print_settings
   --  (print, settings);
   ...
   --  gtk_window_present (GTK_WINDOW (error_dialog)); g_error_free (error); }
   --  else if (res == GTK_PRINT_OPERATION_RESULT_APPLY) { if (settings !=
   ...
   --  Gtk.Print_Operation.Print_Operation_Result_Apply indicates that the
   --  printing was completed successfully. In this case, it is a good idea to
   --  obtain the used print settings with
   --  [methodGtk.PrintOperation.get_print_settings] and store them for reuse
   --  with the next print operation. A value of
   --  Gtk.Print_Operation.Print_Operation_Result_In_Progress means the
   ...
   procedure Set_Allow_Async
      (Self        : not null access Gtk_Print_Operation_Record;
       Allow_Async : Boolean);
   --  Sets whether Gtk.Print_Operation.Run may return before the print
   ...
   --  If this is called before [methodGtk.PrintOperation.run], the user will
   ...
   --  Sets up the `GtkPrintOperation` to generate a file instead of showing
   --  the print dialog.
   ...
   --  may be set in a [signalGtk.PrintOperation::begin-print] signal handler.
   --  Note that the page numbers passed to the
   --  [signalGtk.PrintOperation::request-page-setup] and
   --  [signalGtk.PrintOperation::draw-page] signals are 0-based, i.e. if the
   ...
   --  from `GtkPrintContext` puts the origin at the top left corner of the
   --  page.
   ...
   --  will return Gtk.Print_Operation.Print_Operation_Result_In_Progress as
   --  the status, and emit the [signalGtk.PrintOperation::done] signal when
   --  the operation is actually done.
   --
   --  The Windows port does not support asynchronous operation at all (this
   --  is unlikely to change). On other platforms, all actions except for
   --  Gtk.Print_Operation.Print_Operation_Action_Export support asynchronous
   --  operation.

   Current_Page_Property : constant Glib.Properties.Property_Int;
   --  The current page in the document.
   --
   --  If this is set before [methodGtk.PrintOperation.run], the user will be
   ...
   --
   --  Note that this value is set during print preparation phase
   --  (Gtk.Enums.Print_Status_Preparing), so this value should never be get
   --  before the data generation phase
   --  (Gtk.Enums.Print_Status_Generating_Data). You can connect to the
   --  [signalGtk.PrintOperation::status-changed] signal and call
   --  [methodGtk.PrintOperation.get_n_pages_to_print] when print status is
   --  Gtk.Enums.Print_Status_Generating_Data.
   --
   --  This is typically used to track the progress of print operation.
   ...
   --  from a previous print operation, see [methodGtk.PrintOperation.run].
   ...
   Status_Property : constant Gtk.Enums.Property_Gtk_Print_Status;
   ...
   --  e.g. in a `GtkStatusbar`.
   --
   --  See the [propertyGtk.PrintOperation:status] property for a status value
   ...
   --  of the print job in the printer queues and printer.
   --
   --  This can allow your application to show things like "out of paper"
   --  issues, and when the print job actually reaches the printer. However,
   --  this is often implemented using polling, and should not be enabled
   --  unless needed.

   Unit_Property : constant Gtk.Enums.Property_Gtk_Unit;
   --  The transformation for the cairo context obtained from
   --  `GtkPrintContext` is set up in such a way that distances are measured in
   --  units of Unit.
   ...
   --  [classGtk.PrintContext] and paginate the document accordingly, and then
   --  set the number of pages with [methodGtk.PrintOperation.set_n_pages].
   ...
   --
   --  If you return a widget in a handler for this signal it will be added to
   --  a custom tab in the print dialog. You typically return a container
   --  widget with multiple widgets in it.
   --
   --  The print dialog owns the returned widget, and its lifetime is not
   --  controlled by the application. However, the widget is guaranteed to stay
   --  around until the [signalGtk.PrintOperation::custom-widget-apply] signal
   ...
   --  Result is Gtk.Print_Operation.Print_Operation_Result_Error then you can
   --  call [methodGtk.PrintOperation.get_error] for more information.
   --
   --  If you enabled print status tracking then
   --  [methodGtk.PrintOperation.is_finished] may still return False after the
   ...
   --  [methodGtk.PrintContext.get_cairo_context].
   --
   --  ```c static void draw_page (GtkPrintOperation *operation,
   --  GtkPrintContext *context, int page_nr, gpointer user_data) { cairo_t
   --  *cr; PangoLayout *layout; double width, text_height; int layout_height;
   --  PangoFontDescription *desc; cr = gtk_print_context_get_cairo_context
   --  (context); width = gtk_print_context_get_width (context);
   --  cairo_rectangle (cr, 0, 0, width, HEADER_HEIGHT); cairo_set_source_rgb
   --  (cr, 0.8, 0.8, 0.8); cairo_fill (cr); layout =
   --  gtk_print_context_create_pango_layout (context); desc =
   ...
   --  (double)layout_height / PANGO_SCALE; cairo_move_to (cr, width / 2,
   --  (HEADER_HEIGHT - text_height) / 2); pango_cairo_show_layout (cr,
   --  layout); g_object_unref (layout); } ```
   --
   --  Use [methodGtk.PrintOperation.set_use_full_page] and
   --  [methodGtk.PrintOperation.set_unit] before starting the print operation
   ...
   --  allocated in the [signalGtk.PrintOperation::begin-print] handler.
   ...
   --  [methodGtk.PrintOperation.set_n_pages], and return True if the document
   ...
   --  context with [methodGtk.PrintContext.set_cairo_context].
   --
   --  The custom preview implementation can use
   --  [methodGtk.PrintOperationPreview.is_selected] and
   --  [methodGtk.PrintOperationPreview.render_page] to find pages which are
   --  selected for print and render them. The preview must be finished by
   --  calling [methodGtk.PrintOperationPreview.end_preview] (typically in
   ...
   --  changes done to Setup will be in force only for printing this page.
   -- 
   --  Callback parameters:
   --    --  @param Context the `GtkPrintContext` for the current operation
   --    --  @param Page_Nr the number of the currently printed page (0-based)
   ...
   --
   --  See [enumGtk.PrintStatus] for the phases that are being discriminated.
   --  Use [methodGtk.PrintOperation.get_status] to find out the current
   --  status.
