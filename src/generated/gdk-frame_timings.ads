...
--  [methodGdk.FrameClock.get_timings] or
--  [methodGdk.FrameClock.get_current_timings]. The information in
...
package Gdk.Frame_Timings is
   ...
   --  incrementally as the frame as drawn and passed off to the window system
   --  for processing and display to the user. The accessor functions for
   --  `GdkFrameTimings` can return 0 to indicate an unavailable value for two
   --  reasons: either because the information is not yet available, or because
   ...
   --  the frame. See [methodGdk.FrameClock.get_frame_time].
   ...
   --  [methodGdk.FrameTimings.get_presentation_time], which is only available
   --  after the frame has been presented.
   --  In general, if you are simply animating, you should use
   --  [methodGdk.FrameClock.get_frame_time] rather than this function, but
   ...
   --  [methodGdk.FrameTimings.get_complete]
