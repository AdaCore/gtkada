...
package Gtk.Image is
   ...
   --  add its own reference rather than adopting yours. Note that the animation
   --  frames are shown using a timeout with G_PRIORITY_DEFAULT. When using
   --  animations to indicate busyness, keep in mind that the animation will
   --  only be shown if the main loop is not busy with something that has a
   --  higher priority.
   ...
   procedure Initialize
      (Self     : access Gtk_Image_Record'Class;
       Icon_Set : Gtk.Icon_Factory.Gtk_Icon_Set;
       Size     : Gtk.Enums.Gtk_Icon_Size);
   --  Creates a Gtk.Image.Gtk_Image displaying an icon set. Sample stock sizes
   --  are GTK_ICON_SIZE_MENU, GTK_ICON_SIZE_SMALL_TOOLBAR. Instead of using
   --  this function, usually it's better to create a
   ...
   --  GdkImage is a client-side image buffer in the pixel format of the current
   --  display. The Gtk.Image.Gtk_Image does not assume a reference to the image
   --  or mask; you still need to unref them if you own references.
   ...
   --  created will not react to state changes. Should you want that, you should
   --  use Gtk.Image.Gtk_New.
   ...
   procedure Initialize
      (Self     : access Gtk_Image_Record'Class;
       Stock_Id : UTF8_String;
       Size     : Gtk.Enums.Gtk_Icon_Size);
   --  Creates a Gtk.Image.Gtk_Image displaying a stock icon. Sample stock icon
   --  names are GTK_STOCK_OPEN, GTK_STOCK_QUIT. Sample stock sizes are
   ...
   procedure Get
      (Self   : access Gtk_Image_Record;
       ...
       Mask   : out Gdk.Bitmap.Gdk_Bitmap);
   --  Gets the pixmap and mask being displayed by the Gtk.Image.Gtk_Image. The
   --  storage type of the image must be %GTK_IMAGE_EMPTY or %GTK_IMAGE_PIXMAP
   --  (see Gtk.Image.Get_Storage_Type). The caller of this function does not
   --  own a reference to the returned pixmap and mask.
   ...
   --  Gets the type of representation being used by the Gtk.Image.Gtk_Image to
   --  store image data. If the Gtk.Image.Gtk_Image has no image data, the
   ...
   --  the #GtkImage:icon-size property for images of type %GTK_IMAGE_ICON_NAME.
   -- 
