...
package Gtk.Text_Tag_Table is
   ...
   --  A function used with Gtk.Text_Tag_Table.Foreach, to iterate over every
   ...
   --  If a `GtkTextBuffer` has Table as its tag table, the tag is removed
   --  from the buffer. The table's reference to the tag is removed, so the tag
   --  will end up destroyed if you don't have a reference to it.
   --  @param Tag a `GtkTextTag`
   ...
   --    --  @param Size_Changed whether the change affects the `GtkTextView`
   --    --  layout.
