...
package Glib.IOChannel is
   ...
   procedure G_New_File
      (Self     : out Giochannel;
       Filename : UTF8_String;
       Mode     : UTF8_String);
   --  Open a file Filename as a Glib.IOChannel.Giochannel using mode Mode.
   --  This channel will be closed when the last reference to it is dropped, so
   --  there is no need to call g_io_channel_close (though doing so will not
   --  cause problems, as long as no attempt is made to access the channel
   --  after it is closed).
   --  "filename": A string containing the name of a file
   --  "mode": One of "r", "w", "a", "r+", "w+", "a+". These have the same
   --  meaning as in fopen

   function Giochannel_New_File
      (Filename : UTF8_String;
       Mode     : UTF8_String) return Giochannel;
   --  Open a file Filename as a Glib.IOChannel.Giochannel using mode Mode.
   --  This channel will be closed when the last reference to it is dropped, so
   --  there is no need to call g_io_channel_close (though doing so will not
   --  cause problems, as long as no attempt is made to access the channel
