...
package Gtk.Text_Iter is
   ...
   --
   --  If neither `GTK_TEXT_SEARCH_VISIBLE_ONLY` nor
   --  `GTK_TEXT_SEARCH_TEXT_ONLY` are enabled, the match must be exact; the
   --  special 0xFFFC character will match embedded paintables or child
   --  widgets.
   ...
   type Gtk_Text_Char_Predicate is access function (Ch : Gunichar) return Boolean;
   --  The predicate function used by gtk_text_iter_forward_find_char and
   --  Gtk.Text_Iter.Backward_Find_Char.
   ...
   procedure Assign (Iter : in out Gtk_Text_Iter; Other : Gtk_Text_Iter);
   ...
   --  when writing loops.
   --  @param Result whether movement was possible
   ...
   --  @param Result whether Iter moved and is dereferenceable

   procedure Backward_Cursor_Position
      (Iter   : in out Gtk_Text_Iter;
       Result : out Boolean);
   --  Like [methodGtk.TextIter.forward_cursor_position], but moves backward.
   --  @param Result True if we moved
   ...
   --  @param Result True if we moved and the new position is dereferenceable

   function Backward_Find_Char
      (Iter  : in out Gtk_Text_Iter;
       Pred  : Gtk_Text_Char_Predicate;
       Limit : Gtk_Text_Iter := Null_Text_Iter) return Boolean;
   --  Same as [methodGtk.TextIter.forward_find_char], but goes backward from
   --  Iter.
   ...
   --  line 0, but not at the start of the line, Iter is snapped to the start
   --  of the line and the function returns True. (Note that this implies that
   --  in a loop calling this function, the line number may not change on every
   --  iteration, if your first iteration is on line 0.)
   --  @param Result whether Iter moved
   ...
   procedure Backward_Search
      (Iter        : in out Gtk_Text_Iter;
       ...
       Result      : out Boolean);
   --  Same as [methodGtk.TextIter.forward_search], but moves backward.
   --  Match_End will never be set to a `GtkTextIter` located after Iter, even
   --  if there is a possible Match_Start before or at Iter.
   ...
   --  of buffer
   --  @param Result whether a match was found

   procedure Backward_Sentence_Start
      (Iter   : in out Gtk_Text_Iter;
       Result : out Boolean);
   --  Moves backward to the previous sentence start.
   --  If Iter is already at the start of a sentence, moves backward to the
   --  next one.
   --  Sentence boundaries are determined by Pango and should be correct for
   --  nearly any language.
   --  @param Result True if Iter moved and is not the end iterator

   procedure Backward_Sentence_Starts
      (Iter   : in out Gtk_Text_Iter;
       Count  : Glib.Gint;
       Result : out Boolean);
   --  Calls [methodGtk.TextIter.backward_sentence_start] up to Count times.
   ...
   procedure Backward_To_Tag_Toggle
      (Iter   : in out Gtk_Text_Iter;
       ...
       Result : out Boolean);
   --  Moves backward to the next toggle (on or off) of the Tag, or to the
   --  next toggle of any tag if Tag is null.
   --  If no matching tag toggles are found, returns False, otherwise True.
   --  Does not return toggles located at Iter, only toggles before Iter. Sets
   --  Iter to the location of the toggle, or the start of the buffer if no
   --  toggle is found.
   --  @param Tag a `GtkTextTag`
   --  @param Result whether we found a tag toggle before Iter
   ...
   procedure Backward_Visible_Word_Starts
      (Iter   : in out Gtk_Text_Iter;
       Count  : Glib.Gint;
       Result : out Boolean);
   --  Calls [methodGtk.TextIter.backward_visible_word_start] up to Count
   --  times.
   ...
   procedure Backward_Word_Starts
      (Iter   : in out Gtk_Text_Iter;
       Count  : Glib.Gint;
       Result : out Boolean);
   --  Calls [methodGtk.TextIter.backward_word_start] up to Count times.
   --  @param Count number of times to move
   --  @param Result True if Iter moved and is not the end iterator
   ...
   --  [methodGtk.TextBuffer.insert_interactive] uses this function to decide
   ...
   --  [methodGtk.TextIter.can_insert] to handle this case.
   ...
   --  end of the tagged range, but that the character at Iter is outside the
   --  tagged range. In other words, unlike [methodGtk.TextIter.starts_tag], if
   --  this function returns True, [methodGtk.TextIter.has_tag] will return
   ...
   --  offsets yourself. Also, it's a bit faster than
   --  [methodGtk.TextIter.compare].
   --  @param Rhs another `GtkTextIter`
   ...
   --  this function may actually move onto an image instead of a character, if
   --  you have images in your buffer. If Iter is the end iterator or one
   --  character before it, Iter will now point at the end iterator, and this
   --  function returns False for convenience when writing loops.
   --  @param Result whether Iter moved and is dereferenceable
   ...
   --  If Count would move past the start or end of the buffer, moves to the
   --  start or end of the buffer.
   --  The return value indicates whether the new position of Iter is
   ...
   --  See also the [structPango.LogAttr] struct and the [funcPango.break]
   --  function.
   --  @param Result True if we moved and the new position is dereferenceable
   ...
   --  returns True, Iter is set to Limit if Limit is non-null, otherwise to
   --  the end iterator.
   ...
   package Forward_Find_Char_User_Data is

      type Gtk_Text_Char_Predicate is access function (Ch : Gunichar; User_Data : User_Data_Type) return Boolean;
      --  The predicate function used by Gtk.Text_Iter.Forward_Find_Char and
      --  Gtk.Text_Iter.Backward_Find_Char.
      --  @param Ch a Unicode code point
      ...
      function Forward_Find_Char
         (Iter      : in out Gtk.Text_Iter.Gtk_Text_Iter;
          ...
          Limit     : Gtk.Text_Iter.Gtk_Text_Iter := Null_Text_Iter)
          return Boolean;
      --  Advances Iter, calling Pred on each character.
      --  If Pred returns True, returns True and stops scanning. If Pred never
      --  returns True, Iter is set to Limit if Limit is non-null, otherwise to
   ...
   end Forward_Find_Char_User_Data;
   ...
   --  the end of the buffer and not dereferenceable, returns False. Otherwise,
   --  returns True.
   --  @param Result whether Iter can be dereferenced
   ...
   procedure Forward_Sentence_Ends
      (Iter   : in out Gtk_Text_Iter;
       Count  : Glib.Gint;
       Result : out Boolean);
   --  Calls [methodGtk.TextIter.forward_sentence_end] Count times.
   ...
   --  [methodGtk.TextIter.get_char] called on the end iterator returns 0,
   ...
   --  to the paragraph delimiter characters for the next line. If Iter is on
   --  the last line in the buffer, which does not end in paragraph delimiters,
   --  moves to the end iterator (end of the last line), and returns False.
   --  @param Result True if we moved and the new location is not the end
   --  iterator
   ...
   procedure Forward_To_Tag_Toggle
      (Iter   : in out Gtk_Text_Iter;
       Tag    : Gtk.Text_Tag.Gtk_Text_Tag := null;
       Result : out Boolean);
   --  Moves forward to the next toggle (on or off) of the Tag, or to the next
   --  toggle of any tag if Tag is null.
   --  If no matching tag toggles are found, returns False, otherwise True.
   --  Does not return toggles located at Iter, only toggles after Iter. Sets
   --  Iter to the location of the toggle, or to the end of the buffer if no
   --  toggle is found.
   --  @param Tag a `GtkTextTag`
   --  @param Result whether we found a tag toggle after Iter
   ...
   procedure Forward_Visible_Word_Ends
      (Iter   : in out Gtk_Text_Iter;
       Count  : Glib.Gint;
       Result : out Boolean);
   --  Calls [methodGtk.TextIter.forward_visible_word_end] up to Count times.
   --  @param Count number of times to move
   --  @param Result True if Iter moved and is not the end iterator
   ...
   procedure Forward_Word_Ends
      (Iter   : in out Gtk_Text_Iter;
       Count  : Glib.Gint;
       Result : out Boolean);
   --  Calls [methodGtk.TextIter.forward_word_end] up to Count times.
   --  @param Count number of times to move
   --  @param Result True if Iter moved and is not the end iterator
   ...
   --  identical to that of [funcGtk.get_default_language].
   --  @return language in effect at Iter. Has transfer-ownership='full'.
   ...
   --  If Line_Number is negative or larger than or equal to the number of
   --  lines in the buffer, moves Iter to the start of the last line in the
   --  buffer.
   ...
   --  Remember that `GtkTextBuffer` encodes text in UTF-8, and that
   ...
   --  Same as [methodGtk.TextIter.set_line_offset], but works with a byte
   --  index. The given byte index must be at the start of a character, it
   --  can't be in the middle of a UTF-8 encoded character.
   ...
   --  line. See [methodGtk.TextIter.set_line_index] if you have a byte index
   ...
   --  Each character in a `GtkTextBuffer` has an offset, starting with 0 for
   --  the first character in the buffer. Use
   --  [methodGtk.TextBuffer.get_iter_at_offset] to convert an offset back into
   --  an iterator.
   ...
   function Get_Paintable
      (Iter : Gtk_Text_Iter) return Gdk.Paintable.Gdk_Paintable;
   pragma Import (C, Get_Paintable, "gtk_text_iter_get_paintable");
   ...
   --  The `GtkTextTag`s in the list don't have a reference added, but you
   --  have to free the list itself.
   ...
   --  Like [methodGtk.TextIter.set_line_index], but the index is in visible
   ...
   --  Like [methodGtk.TextIter.set_line_offset], but the offset is in visible
   ...
   --  Like [methodGtk.TextIter.get_slice], but invisible text is not
   --  included. Invisible text is usually invisible because a `GtkTextTag`
   ...
   function Get_Visible_Text
      (Iter    : Gtk_Text_Iter;
       The_End : Gtk_Text_Iter) return UTF8_String;
   --  Returns visible text in the given range.
   --  Like [methodGtk.TextIter.get_text], but invisible text is not included.
   ...
   --  tagged with Tag.
   --  See also [methodGtk.TextIter.starts_tag] and
   --  [methodGtk.TextIter.ends_tag].
   --  @param Tag a `GtkTextTag`
   ...
   --  Word breaks are determined by Pango and should be correct for nearly
   --  any language.
   --  Note that if [methodGtk.TextIter.starts_word] returns True, then this
   ...
   --  See [methodGtk.TextIter.forward_cursor_position] or
   --  [structPango.LogAttr] or [funcPango.break] for details on what a cursor
   --  position is.
   ...
   --  buffer. [methodGtk.TextIter.is_end] is the most efficient way to check
   ...
   --  buffer.
   --  That is, ensures that First and Second are in sequence. Most text
   ...
   --  There are some exceptions, such as [methodGtk.TextIter.in_range], that
   --  expect a pre-sorted range.
   --  @param Second another `GtkTextIter`

   function Starts_Line (Iter : Gtk_Text_Iter) return Boolean;
   --  Returns True if Iter begins a paragraph.
   --  This is the case if [methodGtk.TextIter.get_line_offset] would return
   --  0. However this function is potentially more efficient than
   --  [methodGtk.TextIter.get_line_offset], because it doesn't have to compute
   ...
   --  Note that if this function returns True, it means that Iter is at the
   --  beginning of the tagged range, and that the character at Iter is inside
   --  the tagged range. In other words, unlike [methodGtk.TextIter.ends_tag],
   --  if this function returns True, [methodGtk.TextIter.has_tag] will also
