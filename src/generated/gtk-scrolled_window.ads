...
--  <description>
--  GtkScrolledWindow is a Gtk.Bin.Gtk_Bin subclass: it's a container the
--  accepts a single child widget. GtkScrolledWindow adds scrollbars to the
--  child widget and optionally draws a beveled frame around the child widget.
--
...
--  GtkScrolledWindow with Gtk.Container.Add. If a widget does not, you must
--  first add the widget to a Gtk.Viewport.Gtk_Viewport, then add the
--  Gtk.Viewport.Gtk_Viewport to the scrolled window. Gtk.Container.Add will do
--  this for you for widgets that don't implement Gtk.Scrollable.Gtk_Scrollable
--  natively, so you can ignore the presence of the viewport.
--
...
--  and the Gtk.Viewport.Gtk_Viewport from the GtkScrolledWindow with either of
--  the calls |[<!-- language="C" --> gtk_container_remove (GTK_CONTAINER
--  (scrolled_window), child_widget); // or gtk_container_remove (GTK_CONTAINER
--  (scrolled_window), gtk_bin_get_child (GTK_BIN (scrolled_window))); ]|
--
...
--  Gtk.Scrollbar.Gtk_Scrollbar, used by GtkScrolledWindow, the "value" field
--  represents the position of the scrollbar, which must be between the "lower"
--  field and "upper - page_size." The "page_size" field represents the size of
--  the visible scrollable area. The "step_increment" and "page_increment"
--  fields are used when the user asks to step down (using the small stepper
...
package Gtk.Scrolled_Window is
   ...
   --  propagated through the scrolled windows requested natural height.
   --  Since: gtk+ 3.22
   ...
   --  propagated through the scrolled windows requested natural width.
   --  Since: gtk+ 3.22
