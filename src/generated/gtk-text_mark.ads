...
--  Marks a position in a `GtkTextbuffer` that is preserved across
--  modifications.
--
...
--  with [methodGtk.TextBuffer.delete_mark]. Once deleted from the buffer, a
...
package Gtk.Text_Mark is
   ...
   procedure Initialize
      (Mark         : not null access Gtk_Text_Mark_Record'Class;
       ...
       Left_Gravity : Boolean);
   --  Creates a text mark.
   --  Add it to a buffer using [methodGtk.TextBuffer.add_mark]. If Name is
   --  null, the mark is anonymous; otherwise, the mark can be retrieved by
   --  name using [methodGtk.TextBuffer.get_mark]. If a mark has left gravity,
   --  and text is inserted at the mark's current location, the mark will be
   --  moved to the left of the newly-inserted text. If the mark has right
   ...
   --  See [methodGtk.TextBuffer.add_mark] for a way to add it to a buffer
   --  again.
   ...
   --
   --  When text is inserted at the mark's current location, if the mark has
   --  left gravity it will be moved to the left of the newly-inserted text,
