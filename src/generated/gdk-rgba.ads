...
--  represents transparent black and (1.0, 1.0, 1.0, 1.0) is opaque white.
...
package Gdk.RGBA is
   ...
   --  cairo's way to deal with colors and mirrors its behavior. All values are
   --  in the range from 0.0 to 1.0 inclusive. So the color (0.0, 0.0, 0.0,
   --  0.0) represents transparent black and (1.0, 1.0, 1.0, 1.0) is opaque
   ...
   function Is_Clear (Self : Gdk_RGBA) return Boolean;
   --  Checks if an Rgba value is transparent.
   --  That is, drawing with the value would not produce any change.
   --  @return True if the Rgba is clear

   function Is_Opaque (Self : Gdk_RGBA) return Boolean;
   ...
   --  - A standard name (Taken from the CSS specification). - A hexadecimal
   --  value in the form "\rgb", "\rrggbb", "\rrrgggbbb" or "\rrrrggggbbbb" - A
   --  hexadecimal value in the form "\rgba", "\rrggbbaa", or
   --  "\rrrrggggbbbbaaaa" - A RGB color in the form "rgb(r,g,b)" (In this case
   --  the color will have full opacity) - A RGBA color in the form
   --  "rgba(r,g,b,a)" - A HSL color in the form "hsl(h,s,l)" - A HSLA color in
   --  the form "hsla(h,s,l,a)"
   ...
   --  100%, and a is a floating point value in the range 0 to 1. The range for
   ...
   function Print
      (Self   : Gdk_RGBA;
       String : Glib.String.Gstring) return Glib.String.Gstring;
   pragma Import (C, Print, "gdk_rgba_print");
   ...
   --  colors module, and can be parsed by [methodGdk.RGBA.parse].
