...
package body Gtk.Print_Settings is
   ...
   begin
      Self := new Gtk_Print_Settings_Record;
      Gtk.Print_Settings.Initialize_From_File (Self, File_Name, Error);
   ...
   begin
      Self := new Gtk_Print_Settings_Record;
      Gtk.Print_Settings.Initialize_From_Key_File (Self, Key_File, Group_Name, Error);
   ...
   is
      function Internal
         (File_Name : Gtkada.Types.Chars_Ptr;
   ...
   package body Foreach_User_Data is
      ...
      --  Function called by [methodGtk.PrintSettings.foreach] on every
      --  key/value pair inside a [classGtk.PrintSettings].
      --  @param Key the setting key
      --  @param Value the setting value
      --  @param User_Data The user data provided with the function
   ...
   is
      ...
      Tmp_Return    : Glib.Gboolean;
   begin
      Tmp_Return := Internal (Get_Object (Self), Tmp_File_Name, Acc_Error'Access);
      Error := Acc_Error;
      Free (Tmp_File_Name);
      return Tmp_Return /= 0;
   end Load_File;
   ...
   begin
      ...
      Tmp_Return := Internal (Get_Object (Self), Key_File, Tmp_Group_Name, Acc_Error'Access);
      Error := Acc_Error;
      Free (Tmp_Group_Name);
      return Tmp_Return /= 0;
   end Load_Key_File;
