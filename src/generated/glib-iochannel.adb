   ...
   is
      function Internal
         (Filename : Gtkada.Types.Chars_Ptr;
          Mode     : Gtkada.Types.Chars_Ptr) return Giochannel;
      pragma Import (C, Internal, "g_io_channel_new_file");
      Tmp_Filename : Gtkada.Types.Chars_Ptr := New_String (Filename);
      Tmp_Mode     : Gtkada.Types.Chars_Ptr := New_String (Mode);
      Tmp_Return   : Giochannel;
   begin
      Tmp_Return := Internal (Tmp_Filename, Tmp_Mode);
      Free (Tmp_Mode);
      Free (Tmp_Filename);
      Self := Tmp_Return;
   end G_New_File;
