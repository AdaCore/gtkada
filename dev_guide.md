...
produced by `contrib/binding.py` from the `.gir` files and the
per-package `.toml` overrides under `contrib/binding/packages/`, and
`make generate` erases and rewrites them, so any manual change is lost on
the next regeneration. When a generated subprogram is wrong (for example
a `GdkRGBA*` bound by value instead of by pointer), fix the corresponding
`.toml` override — typically a `body` key on a `[[method]]` entry — and
run `make generate`. The fixed output is then reproducible. See
`GtkColorDialog.toml` / `GtkColorDialogButton.toml` for worked examples
of a hand-written `body` correcting a pointer ABI.
