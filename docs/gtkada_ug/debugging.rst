...
*compile glib and GTK with `--enable-debug=yes`*
  ...
  *GOBJECT_DEBUG=objects:signals*
    This sets up the debugging output for glib. The value ``objects``
    ...
    second value ``signals`` will display all the signals emitted by
  ...
  *GDK_DEBUG=updates:nograbs:events:dnd:misc:xim:colormap:gdkrgb:gc:pixmap:image:input:cursor*
    ...
    ``nograbs``, which prevents the application from ever grabbing the
  ...
  *GTK_DEBUG=misc:plugsocket:text:tree:updates:keybindings*
    ...
    ``keybindings`` might prove useful sometimes.

*keyboard input buffers/freezes while debugging on Windows*
  GTK on Windows installs a low level hook to handle snapping via 
...
*Import the C function ada_gtk_debug_get_ref_count*
  ...
  .. literalinclude:: ../../testsuite/tests/user-guide/ug_debugging.ads
     :language: ada
     :start-after: --  START ref_count
     :end-before: --  END ref_count
     :dedent: 3

  and should be called on a widget in a manner similar to:

  .. literalinclude:: ../../testsuite/tests/user-guide/ug_debugging.adb
     :language: ada
     :start-after: --  START ref_count_call
     :end-before: --  END ref_count_call
     :dedent: 6
