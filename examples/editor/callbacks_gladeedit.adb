...
package body Callbacks_Gladeedit is
   ...
   Open_Filesel, Save_Filesel : Gtk_File_Selection;
   About : Gtk_Widget;
   ...
   procedure New_File (Main_Window : Gtk_Window);
   --  Creates a new document, by clearing the text widget and setting the
   --  current filename to null.

   procedure Open_File (Main_Window : Gtk_Window);
   --  Shows the file selection dialog to open a file

   procedure Save_As (Main_Window : Gtk_Window);
   --  Shows the file selection dialog to save a file

   procedure Real_Open_File
     (Main_Window : Gtk_Window;
      Filename : String);
   --  Loads the given file

   procedure Real_Save_File
     (Main_Window : Gtk_Window;
      Filename : String);
   --  Saves the given file

   procedure Set_Window_Title (Main_Window : Gtk_Window);
   --  Sets the window title according to the current filename.

   procedure New_File (Main_Window : Gtk_Window) is
      Statusbar : Gtk_Statusbar;
   begin
      Gtk.Editable.Delete_Text (Gtk_Editable (Text1), 0, -1);
      Current_Filename := null;
      File_Changed := False;
      Set_Window_Title (Main_Window);

      Gtk.Status_Bar.Pop (Statusbar1, 1);
      Id := Gtk.Status_Bar.Push (Statusbar1, 1, "New file.");
   end New_File;

   procedure Open_File (Main_Window : Gtk_Window) is
   begin
      ...
      Gtk.Widget.Show_All (Gtk_Widget (Open_Filesel));
      Gdk.Window.Gdk_Raise (Get_Window (Open_Filesel));
   end Open_File;

   procedure Save_As (Main_Window : Gtk_Window) is
   begin
      ...
      if Current_Filename /= null then
         Gtk.File_Selection.Set_Filename (Save_Filesel, Current_Filename.all);
      end if;

      Gtk.Widget.Show_All (Gtk_Widget (Save_Filesel));
   ...
   begin
      ...
      if Fp = Invalid_FD then
          Gtk.Text.Thaw (Text1);
          Id := Gtk.Status_Bar.Push (Statusbar1, 1, "Couldn't open file.");
          return;
      end if;
      ...
      loop
         ...
         if Bytes_Read > 0 then
           Gtk.Text.Insert (Text1, Null_Font, Null_Color, Null_Color,
             Buffer, Gint (Bytes_Read));
         end if;
      ...
      Set_Window_Title (Main_Window);
      Gtk.Status_Bar.Pop (Statusbar1, 1);
      Id := Gtk.Status_Bar.Push (Statusbar1, 1, "File opened.");
   end Real_Open_File;
   ...
   procedure Set_Window_Title (Main_Window : Gtk_Window) is
   begin
      if Current_Filename = null then
   ...
   begin
      ...
      if Current_Filename = null then
         ...
         Real_Save_File (Main_Window, Current_Filename.all);
      end if;
   ...
   procedure On_Cut_Activate
     (Object : access Gtk_Menu_Item_Record) is
   begin
      Gtk.Editable.Cut_Clipboard (Gtk_Editable (Text1), 0);

      Gtk.Status_Bar.Pop (Statusbar1, 1);
      Id := Gtk.Status_Bar.Push (Statusbar1, 1, "Text cut to clipboard.");
   end On_Cut_Activate;

   procedure On_Copy_Activate
     (Object : access Gtk_Menu_Item_Record) is
   begin
      Gtk.Editable.Copy_Clipboard (Gtk_Editable (Text1), 0);

      Gtk.Status_Bar.Pop (Statusbar1, 1);
      Id := Gtk.Status_Bar.Push (Statusbar1, 1, "Text copied.");
   end On_Copy_Activate;

   procedure On_Paste_Activate
     (Object : access Gtk_Menu_Item_Record) is
   begin
      Gtk.Editable.Paste_Clipboard (Gtk_Editable (Text1), 0);

      Gtk.Status_Bar.Pop (Statusbar1, 1);
      Id := Gtk.Status_Bar.Push (Statusbar1, 1, "Text pasted.");
   end On_Paste_Activate;

   procedure On_Delete_Activate
     (Object : access Gtk_Menu_Item_Record) is
   begin
      Gtk.Editable.Delete_Selection (Gtk_Editable (Text1));

      Gtk.Status_Bar.Pop (Statusbar1, 1);
      Id := Gtk.Status_Bar.Push (Statusbar1, 1, "Text deleted.");
   end On_Delete_Activate;

   procedure On_About_Activate (Object : access Gtk_Menu_Item_Record) is
   begin
      if About = null then
         About := Gtk_Widget (Create_About_Dialog);
      end if;

      Gtk.Widget.Show_All (About);
   ...
   procedure On_Text_Changed
     (Object : access Gtk_Text_Record) is
   begin
      File_Changed := True;
   end On_Text_Changed;
   ...
   begin
      ...
      Free (Current_Filename);
      Current_Filename :=
        new String'(Gtk.File_Selection.Get_Filename (Open_Filesel));
      Real_Open_File (Main_Window, Current_Filename.all);
   ...
   begin
      ...
      Free (Current_Filename);
      Current_Filename :=
        new String'(Gtk.File_Selection.Get_Filename (Save_Filesel));
      Real_Open_File (Main_Window, Current_Filename.all);
